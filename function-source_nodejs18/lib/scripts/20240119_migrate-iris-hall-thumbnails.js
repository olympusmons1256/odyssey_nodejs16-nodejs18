...
(async () => {
    ...
    const spaces = await (0, firestore_1.getSpaces)(irisHallOrgId);
    ...
    const spaceItems = (await Promise.all(spaces.flatMap(async ([spaceDoc]) => {
        if (spaceDoc == undefined)
            return undefined;
        // if (spaceDoc.id != "2UxH5bjL6QFcfxnfuaPd7V") return undefined;
        return await (0, firestore_1.getSpaceItems)(irisHallOrgId, spaceDoc.id);
    ...
    const spaceItemUpdates = spaceItems.flatMap(([spaceItemDoc, spaceItem]) => {
        if (spaceItemDoc == undefined || spaceItem == undefined)
            return [];
        if (spaceItem.type != "ConfiguratorObject")
            return [];
        if (spaceItem.thumb == undefined)
            return [];
    ...
    const result = await Promise.all(spaceItemUpdates.map(async (spaceItemUpdate) => {
        try {
            ...
            await spaceItemUpdate.doc.ref.update(Object.assign(Object.assign({}, spaceItemUpdate.update), { migratedThumbAt: admin.firestore.Timestamp.now() }));
        }
        catch (e) {
            logerror(e);
...
//# sourceMappingURL=20240119_migrate-iris-hall-thumbnails.js.map