    ...
    .https.onCall(async (data) => {
      try {
        ...
        if (!emailLink) {
          ...
          const templateAlias = user?.pending ? "new-account" : "signin-v2";
...
const anonymousAuthSignin =
  functions
    .runWith(customRunWithWarm)
    .https.onCall(async (data) : Promise<any> => {
      try {
  ...
  functions
    ...
    .https.onCall(async (data) => {
      try {
        ...
        const uniqueFileName = `${avatarId}-${uuidv4()}`; 
        ...
        const avatarImageDownloadURL = await uploadFileUrlToCloudStorage(response.url, "/avatar-images", `${uniqueFileName}.png`)
          .catch((err)=> {
            console.error(err);
            return undefined;
          });
