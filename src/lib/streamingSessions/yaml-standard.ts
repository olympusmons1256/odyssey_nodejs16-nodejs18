...
import {V1NodeSelectorRequirement, V1NodeSelectorTerm, V1PreferredSchedulingTerm, V1Pod} from "@kubernetes/client-node";
...
export function templateStreamingSessionConfigMap(projectId: string, organizationId: string, spaceId: string, roomId: string, userId: string, deviceId: string, deploymentId: string, iceServers: IceServer[], configMapCustomYaml: string) {
  const configMap = yaml.loadAll(configMapCustomYaml)[0] as any;
...
export function templateStreamingSessionPod(
  ...
  if (pod.spec.volumes != undefined) {
    if (workloadClusterProvider == "coreweave" && configuration.unrealMountContent == true && configuration.unrealMountImageId != undefined && configuration.unrealImageId != undefined) {
      ...
      const volumeClaim : any = {
        readOnly: true,
        claimName,
      };
      const contentMount : any = {
  ...
  pod.spec.containers = pod.spec.containers.map((container) => {
    ...
    if (container.name == "unreal") {
      if (configuration.unrealMountContent == true && configuration.unrealMountImageId != undefined && configuration.unrealImageId != undefined) {
        ...
        const odysseyArtContentMounts : any[] = [
          {
            name: "odyssey-content",
        ...
        const thirdPersonTemplateContentMounts : any[] = [
          {
            name: "odyssey-content",
...
export function templateStreamingSessionIngressGke(projectId: string, organizationId: string, spaceId: string, roomId: string, userId: string, deviceId: string, deploymentId: string, ingressYaml: string): any {
  const ingress = yaml.loadAll(ingressYaml)[0] as any;
  ...
  if (projectId == "ngp-odyssey-prod") {
    ingress.spec.tls.map((entry: { secretName?: string; hosts?: string[] }) => {
      ...
      if (entry.hosts != undefined) {
        entry.hosts = entry.hosts.map((host: string) => {
  ...
  ingress.spec.rules.map((rule: { host?: string; http?: { paths: Array<{ backend: unknown; path?: string }> } }) => {
    if (projectId == "ngp-odyssey-prod") {
      rule.host = rule.host?.replace("odyssey-client-dev", "odyssey-client");
    }
    if (rule.http != undefined) {
      rule.http.paths.map((path: { backend: unknown; path?: string }) => {
        if (path.path) {
...
export function templateStreamingSessionIngress(projectId: string, namespace: string, region: string, organizationId: string, spaceId: string, roomId: string, userId: string, deviceId: string, deploymentId: string, ingressYaml: string): any {
  const ingress = yaml.loadAll(ingressYaml)[0] as any;
...
export function templateImagePullDaemonset(daemonsetYaml: string, configuration: ConfigurationOdysseyClientPod): any {
  const daemonset = yaml.loadAll(daemonsetYaml)[0] as any;
