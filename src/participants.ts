...
const newParticipantDenormalize =
  ...
  functions
    ...
    .onCreate(async (snapshot, context) => {
      ...
      const [, user] = await getUser(participantId.split(":")[0]);
      if (user == undefined) {
      ...
      const [, orgMember] = await getOrganizationUser(organizationId, participantId.split(":")[0]);
      const [, spaceMember] = await getSpaceUser(organizationId, spaceId, participantId.split(":")[0]);
    ...
    .onUpdate(async (change, context) => {
      ...
      switch (deploymentAfter.state) {
        case "pod-ready": {
          console.debug(`This deployment ${deploymentId} is ready: `, deploymentAfter);
          // Gather latest state of all deployments
          const [deploymentsDoc] = await getDeployments(deploymentAfter.organizationId, deploymentAfter.roomId, deploymentAfter.participantId);
          if (deploymentsDoc == undefined) {
            return console.error("Failed to get deployments");
          }
          const deployments = deploymentsDoc.data() as docTypes.Deployments;
          if (deployments == undefined) {
          ...
          if (allDeploymentsReady) {
            await updateParticipantState(deploymentAfter.organizationId, deploymentAfter.roomId, deploymentAfter.participantId, "ready");
          }
          break;
        }
      }
