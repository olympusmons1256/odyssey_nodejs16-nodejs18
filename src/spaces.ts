...
import {getSpaceRef, getSpacesRef, spaceWildcardPath, getDerivedSpacesWithUnrealProject, getUnrealProject} from "./lib/documents/firestore";
...
async function updateUnrealProjectSpaceCount(unrealProjectId: string, existingSpace?: {spaceId: string, action: "add" | "delete"}) {
  try {
    ...
    if (derivedSpaceIds.length <= 0) {
      ...
      return unrealProjectDoc.ref.update(toFirestoreUpdateData(updatedUnrealProject));
    }
