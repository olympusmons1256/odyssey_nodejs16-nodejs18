...
async function f() {
  ...
  const gpuRegions = await calculateGpuRegionsFromAvailability(availabilityData, configuration.validRegions, configuration.validGpus, graphicsBenchmark);
  const bestOneRegion = selectWeightEquivalentRandomGpuRegion(gpuRegions);
