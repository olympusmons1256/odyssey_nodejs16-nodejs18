...
export async function deployVolumeCopyPodStacks(
  ...
  if (unrealProjectVersion.packageArchiveUrl == undefined) {
    console.error("unrealProjectVersion.packageArchiveUrl == undefined");
    return await updateUnrealProjectVersionState({unrealProjectId, unrealProjectVersionId, state: "failed-missing-package-archive-url"});
  }
  if (unrealProjectVersion.packageArchiveSha256Sum == undefined) {
    ...
    return await updateUnrealProjectVersionState({unrealProjectId, unrealProjectVersionId, state: "failed-missing-package-archive-checksum"});
  }
