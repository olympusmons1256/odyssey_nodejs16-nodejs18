  ...
  functions
    ...
    .onPublish(async (data, context) => {
      ...
      return await Promise.all([
        getOdysseyClientPodRef().update(updatedClient),
        getOdysseyServerRef().update(updatedServer),
      ]);
    });
...
export const updateCoreweaveAvailability =
  functions
    ...
    .onRun(async (context) => {
      ...
      const loop = async (executions: number) => {
        ...
        if (new Date().getTime() < executedAt + 45000 && executions < 4) {
          console.debug("Sleeping for 10 seconds");
          await sleep(10000);
          console.debug("Looping");
          return await loop(executions + 1);
        } else {
          console.debug("Not enough time left, returning");
          return;
        }
      };
